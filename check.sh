...
SORT_CURRENT_FIRMWARES()
{
    local CURRENT_FILE="$1"
    local TMP
    TMP="$(mktemp)"

    sort -u "$CURRENT_FILE" | LC_ALL=C sort -t/ -k1,1 -k2,2 -k3,3 -k4,4 > "$TMP"
    mv "$TMP" "$CURRENT_FILE"
}
...
APPEND_CURRENT_FIRMWARE()
{
    ...
    if [[ -f "$CURRENT_FILE" ]]; then
        TMP="$(mktemp)"
        {
            printf '%s\n' "$ENTRY"
            cat "$CURRENT_FILE"
        } | sort -u | LC_ALL=C sort -t/ -k1,1 -k2,2 -k3,3 -k4,4 > "$TMP"
        mv "$TMP" "$CURRENT_FILE"
    else
        printf '%s\n' "$ENTRY" > "$CURRENT_FILE"
    fi
...
for i in "product" "vendor"; do
    ...
    ( cd "$TMP_DIR" && 7z a -tzip -mx=0 -mmt="$(nproc --all)" "$FW_OUT_DIR/${LATEST_SHORTVERSION}_$i.zip" "$i.img" ) || exit 1
    rm -f "$TMP_DIR/$i.img"

    echo "Compressing extracted $i"
    ( cd "$FW_OUT_DIR" && 7z a -tzip -mx=0 -mmt="$(nproc --all)" "$FW_OUT_DIR/${LATEST_SHORTVERSION}_$i-extracted.zip" "$i" ) || exit 1
    ...
    if [[ "$i" == "vendor" ]]; then
        ...
        ( cd "$FW_OUT_DIR/vendor" && 7z a -tzip -mx=0 -mmt="$(nproc --all)" "$FW_OUT_DIR/${LATEST_SHORTVERSION}_firmware_tee.zip" "firmware" "tee" ) || exit 1
    elif [[ "$i" == "product" ]]; then
...
if [[ "$RELEASE_EXISTS" == false ]]; then
    gh release create "$TAG" \
        --repo "majaahh/proprietary_vendor_samsung_exynos" \
        --title "$RELEASE_NAME" \
        --notes-file "$FW_OUT_DIR/versions.txt" || exit 1
fi

EXISTING_RELEASE_ASSETS="$(gh release view "$TAG" --repo "$REPO" --json assets --jq '.assets[].name' 2>/dev/null || true)"
